...
def genIndex(number, title):
    ...
    for i in range(30):
        if not os.path.exists(number + '/%d.flv' % i):
            break
        if i == 0:
            htmlconf['playlist'] += u'                    "%d.flv"' % i
        else:
            htmlconf['playlist'] += u',\n'
            htmlconf['playlist'] += u'                    {url: "%d.flv", autoPlay: true}' % i
    ...
    content = template.html.encode('utf-8') % htmlconf
    file.write(content)
    file.close()
...
if __name__ == '__main__':
    ...
    conn = urlopen.open('http://www.wfbrood.com/movie/')
    html = conn.read()
